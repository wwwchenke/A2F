...
use bitvec::{slice::BitSlice, vec::BitVec, prelude::*};
...
use sunscreen_bulletproofs::range_proof_mpc::messages;
use sunscreen_math::{
    poly::Polynomial,
...
use logproof::{
    InnerProductProof, InnerProductProverKnowledge, InnerProductVerifierKnowledge, ProofError, crypto::CryptoHash, linear_algebra::{InnerProduct, Matrix, ScalarMul}, math::{
        InfinityNorm, Log2, ModSwitch, Powers, Tensor, TwosComplementCoeffs, parallel_multiscalar_multiplication, rand256
...
use crate::{
    ...
    public_secret_inner_product::{IPSPInnerProductProof,IPSPProverKnowledge,IPSPVerifierKnowledge},
};
...
pub struct ConsVerifierKnowledge<Q>
where
...
impl<Q> ConsVerifierKnowledge<Q>
where
    Q: Ring + CryptoHash + ModSwitch<ZqRistretto> + RingModulus<4> + Ord,
{
    /**
    ...
    //     let ndb_prime = self.d().checked_mul(self.n()).unwrap().checked_mul(self.b_prime()).unwrap();

    //     total_bounds_all_equations
    //         .checked_add(ndb_prime)
    //         .unwrap()
    // }
        pub fn l(&self) -> u32 {
        let total_bounds_all_equations = self.b_sum();
 ...
 */
pub struct ConsProverKnowledge<Q>
where
    Q: Ring + CryptoHash + ModSwitch<ZqRistretto> + RingModulus<4> + Ord,
{
    /**
     ...
     * Shared knowlege.
     */
    pub vk: ConsVerifierKnowledge<Q>,
}

impl<Q> ConsProverKnowledge<Q>
where
    Q: Ring + ModSwitch<ZqRistretto> + RingModulus<4> + CryptoHash + Ord,
{
    ...
    pub fn new(
        ...
        let vk = ConsVerifierKnowledge::new(a.clone(), t.clone(), f.clone(), bounds.clone());
    ...
    pub fn s_binary_by_index(&self, index: (usize, usize)) -> BitVec {
        ...
        let s_serialized: Vec<ZqRistretto> = ConsProof::serialize(&s_piece, self.vk.d() as usize);
        let b_serialized = ConsProof::serialize_bounds(&b_piece);

        let s_binary: BitVec = ConsProof::to_2s_complement_multibound(&s_serialized, &b_serialized);
...
pub struct ConsProof {
     ...
     */
    pub inner_product_proof_1: InnerProductProof,

    /**
     * The inner product proof for `a`, `z`
     */
    pub inner_product_proof_2: IPSPInnerProductProof,
}
...
impl ConsProof {
    ...
    pub fn create<Q>(
        ...
        pk: &ConsProverKnowledge<Q>,
        g: &[RistrettoPoint],
        h: &[RistrettoPoint],
        u: &RistrettoPoint,
        gens_f: &RistrettoPoint,
        commits: &[CompressedRistretto],
        vec_binding: &[Scalar],
    ) -> Self
    where
        Q: Ring + ModSwitch<ZqRistretto> + Mul<Q, Output = Q> + CryptoHash + RingModulus<4> + Ord,
    {
        ...
        Self::create_with_shared(transcript, pk, g, h, u, &half_rho, &[], gens_f,commits,vec_binding)
    }
    ...
    where
        Q: Ring + ModSwitch<ZqRistretto> + Mul<Q, Output = Q> + CryptoHash + RingModulus<4> + Ord,
    {
        let sub1_proof_start = Instant::now();
        let vk = &pk.vk;
        let d = vk.d();
        let m = vk.m();
        let n = vk.n();
        ...
        let b_serialized = ConsProof::serialize_bounds(&b);
        ...
        let (s_1_shared, h_shared, s_1_unshared, h_unshared) =
            Self::split_small_shared_and_unshared_bits(shared_indices, &b_slices, &s_1, h);
        // let decimal_value: u64 = s_1_shared[..10].load_le::<u64>();
        ...
        let mut transcript_comm = transcript.clone();
        ...
        let inner_product_proof_1 =
            Self::create_inner_product_proof(transcript, &v_1, &v_2, &rho, &t, &g_prime, h, u);
        
        println!("sub1 proof took: {:?}", sub1_proof_start.elapsed());
        let vec_z = Scalar::twos_complement_coeffs(10 as usize);
        let e_1 = &(vec_binding[0] + half_rho);

        let c_ipsp = w_shared + commits[0].decompress().expect("Commits[0] decompression failed");
        let v_1_scalars: Vec<Scalar> = s_1_shared
        .iter()
        .map(|bit| {
            if *bit { Scalar::one() } else { Scalar::zero() }
        })
        .collect();

        let inner_product_proof_2 = Self::create_public_secret_inner_product_proof(transcript, &v_1_scalars, &vec_z, &e_1, &c_ipsp.clone(), &gens_f, &h_shared, &u);
        
        Self {
            w_shared,
            w,
            inner_product_proof_1,
            inner_product_proof_2,
        }
    }
        ...
        Q: Ring + ModSwitch<ZqRistretto> + RingModulus<4> + CryptoHash + Ord,
    {
        ...
        let ip_vk = InnerProductVerifierKnowledge { t, x };

        let mut c_ipsp = self.w_shared.clone();
        let vec_z = Scalar::twos_complement_coeffs(10 as usize);
        let h_shared = &h[0..10];
        c_ipsp = commits.iter()
            .map(|cp| cp.decompress().ok_or(ProofError::VerificationError)) 
            .collect::<Result<Vec<RistrettoPoint>, _>>()?                
            .into_iter()
            .fold(c_ipsp, |acc, p| acc + p);

        let ip_vk_2 = IPSPVerifierKnowledge { t: c_ipsp, z: vec_z };

        self.inner_product_proof_1
            .verify(transcript, &ip_vk, &g_prime, h, u)?;
        println!("sub1 verify took: {:?}", sub1_verify_start.elapsed());
        self.inner_product_proof_2.verify(transcript, &ip_vk_2, &gens_f, h_shared, u)?;
        // self.inner_product_proof_2
        //    .folding_verifier(&mut transcript_commit, &ip_vk_2, &gens_f, &g_prime, &h)?;
        Ok(())
    }
    ...
    fn create_inner_product_proof(
        ...
        let pk = InnerProductProverKnowledge::new(v_1, v_2, rho, t);

        InnerProductProof::create(transcript, &pk, g, h, u)
    }

    #[allow(clippy::too_many_arguments)]
    fn create_public_secret_inner_product_proof(
        transcript: &mut Transcript,
        v_1: &[Scalar],
        v_2: &[Scalar],
        rho: &Scalar,
        ...
        h: &[RistrettoPoint],
        u: &RistrettoPoint,
    ) -> IPSPInnerProductProof {
        assert_eq!(v_1.len(), v_2.len());

        let pk = IPSPProverKnowledge::new(v_1, v_2, rho, t);

        IPSPInnerProductProof::create(transcript, &pk, gens_f, h, u)
    }
        ...
        B: ArithmeticBackend<N>,
    {
        ...
        for value in values.iter() {
            ConsProof::to_2s_complement_single(value, log_b, &mut bitvec);
        }
        ...
        B: ArithmeticBackend<N>,
    {
        ...
        for (value, bound) in zip(values.iter(), log_b.iter()) {
            ConsProof::to_2s_complement_single(value, *bound, &mut bitvec);
        }
    ...
    fn split_small_shared_and_unshared_bits(
        shared_indices: &[(usize, usize)],
        b_slices: &[Vec<Range<usize>>],
        s_1: &BitVec,
        h: &[RistrettoPoint],
        ...
        for i in 0..m {
            for j in 0..k {
                let index = (i, j);
                let range = b_slices[index.0][index.1].clone();

                if shared_index_set.contains(&index) {
                    let mid = std::cmp::min(range.start + 10, range.end);
                    
                    s_1_shared.extend(s_1[range.start..mid].iter());
                    h_shared.extend(h[range.start..mid].iter());

                    if mid < range.end {
                        s_1_unshared.extend(s_1[mid..range.end].iter());
                        h_unshared.extend(h[mid..range.end].iter());
                    }
                } else {
                    s_1_unshared.extend(s_1[b_slices[index.0][index.1].clone()].iter());
                    h_unshared.extend(h[b_slices[index.0][index.1].clone()].iter());
