...
use logproof::{
    ...
    math::ModSwitch,
    rings::ZqRistretto,
    InnerProductVerifierKnowledge,
};

use crate::{
    consistency::ConsVerifierKnowledge,
};

/**
 * Additional operations on Transcripts to perform consistent proofs.
 */
pub trait ConsProofTranscript {
    /**
     ...
     */
    fn append_inner_product_knowledge(&mut self, vk: &InnerProductVerifierKnowledge);

    /**
     * Appends a linear relation problem statement.
     */
    fn append_linear_relation_knowledge<Q>(&mut self, vk: &ConsVerifierKnowledge<Q>)
    where
        Q: Ring + CryptoHash + ModSwitch<ZqRistretto> + RingModulus<4> + Ord;
}

impl ConsProofTranscript for merlin::Transcript {
    fn inner_product_domain_separator(&mut self) {
